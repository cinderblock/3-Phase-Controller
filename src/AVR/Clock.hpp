...
inline void enablePLL(bool const useEXT, Timer4Div const timerDiv, PLLOutFreq const PLL) {
  PLLFRQ = ((useEXT ? 0 : 1) << PINMUX) | (u1(timerDiv) << PLLTM0) | ((PLL == PLLOutFreq::f96MHz ? 1 : 0) << PLLUSB) |
           u1(PLL);
...
inline void enablePLL(bool const useEXT, PLLOutFreq const PLL = defaultPLLOutFreq) {
