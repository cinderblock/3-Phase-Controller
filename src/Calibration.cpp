...
void Calibration::main() {
  ...
  SOUT << Printer::Special::Start << u1(0) << u4(steps) << u1(amplitude) << Printer::Special::End;
  ...
  for (i = 0; i < steps; i += stepSize) {
    ...
    SOUT << Printer::Special::Start << u1(1) << i << MLX90363::getAlpha() << HallWatcher::getState() << s1(1)
         << Printer::Special::End;
  }
...
inline libCameron::DecPrintFormatter &operator<<(libCameron::DecPrintFormatter &out,
                                                 const MLX90363::ResponseState res) {
  switch (res) {
  case MLX90363::ResponseState::Init:
    return out << PSTR("Init");
  case MLX90363::ResponseState::Ready:
    return out << PSTR("Ready");
  case MLX90363::ResponseState::Receiving:
    return out << PSTR("Receiving");
  case MLX90363::ResponseState::Received:
    return out << PSTR("Received");
  case MLX90363::ResponseState::failedCRC:
    return out << PSTR("failedCRC");
  case MLX90363::ResponseState::TypeA:
    return out << PSTR("TypeA");
  case MLX90363::ResponseState::TypeAB:
    return out << PSTR("TypeAB");
  case MLX90363::ResponseState::TypeXYZ:
    return out << PSTR("TypeXYZ");
  case MLX90363::ResponseState::Other:
    return out << PSTR("Other");
  default:
    return out;
  }
}
