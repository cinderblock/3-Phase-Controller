...
static inline void sendSPI(u1 const b) { *SPI::DR = b; }

static inline u1 receiveSPI() { return *SPI::DR; }

void SPI_STC_vect() { MLX90363::isr(); }
...
// Set ROLL to a number that the MLX could never set it to so that handling
// first roll is easy
...
void MLX90363::setSPISpeed(const u1 c) { AVR::SPI::CR->Divider = c; }
...
inline static u1 lookupCRC(u1 const b) { return pgm_read_byte(&cba_256_TAB[b]); }
