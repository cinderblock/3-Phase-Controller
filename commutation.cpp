...
//     // see excel
//     // int commandR[8] = {0, 20, 33, 17, 10, 12, 34, 0 };  // counter
//     clockwise
//     // int commandF[8] = {0, 34, 12, 10, 17, 33, 20, 0 };  // clockwise
//     //                    0   1   2   3   4   5   6  7
//     static ThreePhaseDriver::PhasePosition command[8] = {0,
//     256,0,64,448,320,576,0}
//
//    return ;
// }
using namespace ThreePhaseControllerNamespace;

static const ThreePhaseDriver::PhasePosition command[] = {0,   256, 0,  128,
                                                          512, 384, 640};

void setPWM(signed int amplitude) {
  // Debug::dout << PSTR("setPWM ");
  if (amplitude < 0) {
...
void setOverrideHallState(u1 state) { overrideHallState = state; }
...
void updateCommutation(bool force) {
  ...
  // Debug::dout << PSTR("Converted hall state is angle: ") << angle <<
  // PSTR("\r\n");
  ...
  // Debug::dout << PSTR("Angle with 90deg phase shift:  ") << angle <<
  // PSTR("\r\n"); Debug::dout << PSTR("angle is ") <<
  // ThreePhaseDriver::StepsPerCycle / 4 << PSTR("\r\n");

  // Debug::dout << PSTR("advance To");
